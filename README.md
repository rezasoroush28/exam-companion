...
`node tests/browser/production.cjs` checks the hidden developer-answer controls against a separately started Production server on port 5176. It also removes only its own session. The host forcibly disables `ShowDeveloperAnswers` outside Development, so the engine omits the answer key from question DTOs even if the configuration switch is true. Feedback still reveals correctness after a submitted answer.

The Release publish succeeded. The Production browser check remains unverified: running unpublished output in Production lacked framework static assets, and the environment blocked launching the published test host. Run that check against published output with its connection string pointing to the prepared database; it is not part of the passing Development smoke result.

## Visual and audio choices / limitations

The gramophone combines the approved papercraft artwork with live SVG parts in one coordinate system. A locally stored image supplies the rounded burgundy base, recessed vinyl surface, and soft cream/ochre horn with its curved teal neck. An SVG silhouette clip removes the image background. Five interactive colored grooves, the record label, scratches, playhead, tone arm, and stylus remain state-driven SVG elements. Only the record grooves and label rotate; the base and horn stay fixed. Localized cracks preserve the rest of each groove; successful repair closes the same defect. Home reuses this object in preview mode; the completed album gives it more space. See [gramophone art notes](docs/gramophone-art.md) for the asset and projection details.
...
`gameAudio.js` schedules audio and returns a timeline. `gameMotion.js` samples that same audio clock with requestAnimationFrame; there are no independent CSS/audio playback timers or per-frame server calls. During topic/album playback, unfinished or scratched topics begin with their chord, break up under filtered noise from 1.25s to 2.35s, then recover. The full chord sustains through that 1.1-second interruption so the damage is clearly audible. Clean completed/repaired topics play smoothly; short correct-answer notes stay clean. Record phase aligns each visible scratch with the needle at the interruption onset, and the waveform/needle react for the same duration. Repair replays the same motif without the defect. Playback stops on replacement, pause, hidden tab, or component disposal, with generation guards against stale async work. Reduced motion disables rotation/wobble/jumps while keeping static progress and contact feedback.
