...
The controller owns the 13-stage articulated entrance, semantic Web Animations, and a sparse idle timer. The prototype-derived walk, sit, pocket, and disc-hold segment uses one cancellable absolute-time `requestAnimationFrame` runner; the remaining placement and gameplay reactions retain tracked Web Animations. Generation tokens prevent stale completions from mutating replacements; pause/resume and playback-rate changes preserve current time; root replacement, hidden tabs, navigation, and disposal cleanly cancel the rig. A one-shot sessionStorage marker stages entrance only for newly created sessions, while Home previews the complete prototype segment through the same controller. OS and persisted in-app reduced-motion preferences are combined. Final bow follows completed-album cadence, never answer submission alone. See the [motion/artwork contract](docs/gramophone-art.md) for durations, mappings, lifecycle, and verification.
